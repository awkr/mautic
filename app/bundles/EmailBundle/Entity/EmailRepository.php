    ...
    public function getEmailPendingQuery($emailId, $variantIds = null, $listIds = null, $countOnly = false, $limit = null)
    {
        ...
        if ($variantIds) {
            ...
            if (count($variantIds) == 1) {
                $statExpr->add(
                    $statQb->expr()->eq('stat.email_id', $variantIds[0])
                );
                $messageExpr->add(
                    $mqQb->expr()->eq('mq.channel_id', $variantIds[0])
                );
            } else {
                $statExpr->add(
        ...
        if ($countOnly) {
            ...
            if (count($listIds) == 1) {
                $q->innerJoin('l', MAUTIC_TABLE_PREFIX.'lead_lists_leads', 'll',
                    $q->expr()->andX(
                        $q->expr()->eq('ll.leadlist_id', $listIds[0]),
